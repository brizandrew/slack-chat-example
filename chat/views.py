...
class SlackSlashWebhook(SingleObjectMixin, View):
    ...
    def chat_channel_admin(self, request, channel_id, channel_name):
        """
        Usage: "/chat-channel_admin"
        Returns the channel admin URL, usable only in Chat channels.
        """
        exists, channel_or_resp = self.verify_channel_exists(channel_id)
        if not exists:
            return HttpResponse(channel_or_resp, status=200)
        else:
            channel = channel_or_resp
            if channel_name == 'privategroup':
                channel_name = channel.slug

        pk = channel.pk
        admin_url = settings.SITE_URL + '/admin/chat/chatchannel/{}/change'.format(pk)

        resp = '`{n}` Channel Admin: <{l}|{l}>'.format(
            n=channel_name,
            l=admin_url
        )
        return HttpResponse(resp, status=200)

    def chat_messages_admin(self, request, channel_id, channel_name):
        """
        Usage: "/chat-messages-admin"
        Returns the message admin URL, usable only in Chat channels
        """
        exists, channel_or_resp = self.verify_channel_exists(channel_id)
        if not exists:
            return HttpResponse(channel_or_resp, status=200)
        ...
        admin_url = settings.SITE_URL + '/admin/chat/chatmessage/?channel__slug={}'.format(channel.slug)
        resp = '`{n}` Messages Admin: <{l}|{l}>'.format(
            n=channel_name,
            l=admin_url
        )
        return HttpResponse(resp, status=200)

    def chat_preview(self, request, channel_id, channel_name):
        """
        Usage: "/chat-preview"
        Returns the live preview URL, usable only in Chat channels.
        """
        exists, channel_or_resp = self.verify_channel_exists(channel_id)
        if not exists:
            return HttpResponse(channel_or_resp, status=200)
        ...
        admin_url = settings.SITE_URL + '/live/{}'.format(channel_id)
        resp = '`{n}` Preview: <{l}|{l}>'.format(
            n=channel_name,
            l=admin_url
        )
        return HttpResponse(resp, status=200)
